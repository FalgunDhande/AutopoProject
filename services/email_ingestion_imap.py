...
def validate_credentials():
    ...
    if not EMAIL_USER or not EMAIL_PASS:
        return False, "EMAIL_USER or EMAIL_PASS not found in environment variables."
    
    # Check if credentials are still placeholder values
    if EMAIL_USER == "your-email@gmail.com" or EMAIL_PASS == "your-16-char-app-password":
        return False, "Email credentials are still set to placeholder values."
    
    if "@" not in EMAIL_USER:
        return False, "EMAIL_USER must be a valid email address."
    
    return True, "Credentials validated successfully."

# Validate credentials on startup
is_valid, validation_msg = validate_credentials()
if not is_valid:
    error_msg = f"""
    ╔══════════════════════════════════════════════════════════════════╗
    ...
    ║  {validation_msg:<62} ║
    ║                                                                  ║
    ║  Please update your .env file with valid Gmail credentials:     ║
    ║                                                                  ║
    ...
    ║                                                                  ║
    ║  The service will NOT start until credentials are configured.   ║
    ╚══════════════════════════════════════════════════════════════════╝
    """
    print(error_msg)
...
def run():
    ...
    if not CREDENTIALS_VALID:
        log("Email ingestion service failed to start: Invalid or missing credentials")
        print("\n⚠️  Service stopped. Please configure email credentials in .env file and restart.")
        return
